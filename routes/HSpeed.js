...
router.get('/', function(req, res, next) {
    let sqlString = 'SELECT * FROM STATION WHERE vehicle_type = "HSR"';
    console.log('SELECT');
    // let result = [];
...
router.post('/', function(req, res, next)
{
    ...
    db.run('INSERT INTO ticket(VID,Vehicle_type,Date,start,destination,seat_no,price,type,CID) VALUES(?,"HSR",?,?,?,?,"600",?,?)',generateRandomVID(),req.body.date,req.body.start,req.body.destination,generateRandomSeatNo() , req.body.type, req.body.ID,function(err)
    {
        if(err) console.log(err);
    })
